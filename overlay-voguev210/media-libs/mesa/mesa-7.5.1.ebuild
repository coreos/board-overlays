...
# $Header: /var/cvsroot/gentoo-x86/media-libs/mesa/mesa-7.5.1.ebuild,v 1.6 2009/10/11 11:13:58 nixnut Exp $
...
src_prepare() {
	...
	EPATCH_OPTS="-p1" epatch "${FILESDIR}"/${PV}-pkgconfig.patch
	# FreeBSD 6.* doesn't have posix_memalign().
	[[ ${CHOST} == *-freebsd6.* ]] && \
		sed -i -e "s/-DHAVE_POSIX_MEMALIGN//" configure.ac

	epatch "${FILESDIR}"/samsung-mesa-${PV}.patch
